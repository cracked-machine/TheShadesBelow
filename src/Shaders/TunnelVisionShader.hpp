#ifndef SRC_SHADERS_TUNNELVISIONSHADER_HPP__
#define SRC_SHADERS_TUNNELVISIONSHADER_HPP__
...
//! @brief Full-screen post-process shader that blacks out the already-rendered game frame except for an
//! aperture around the player, which constricts as the player's tachycardia/bradycardia toxicity rises.
//! See Factory::Shader::add_tunnel_vision.
class TunnelVisionShader : public BaseShaderSprite
{
public:
  //! @brief Construct a new Tunnel Vision Shader object and immediately run setup().
  ...
  TunnelVisionShader( std::filesystem::path vert_shader_path, std::filesystem::path frag_shader_path, sf::Vector2u texture_size )
      : BaseShaderSprite( vert_shader_path, frag_shader_path, texture_size )
  {
    setup();
  }
  ~TunnelVisionShader() override = default;
...
#endif // SRC_SHADERS_TUNNELVISIONSHADER_HPP__
